...
const customisation = require('../../data/customisation.json');
  ...
  module.exports ={
    ...
    run: async (client,message,args) =>{
      ...
      if (message.mentions.users.first().id == client.user.id && message.author.id !== "726392596183646260") return message.reply("No kissing unless you're my Dev :<")
      if (message.mentions.users.first().id == message.author.id) return message.reply("Idk if thats possible chief")
      if (message.mentions.users.first().id == client.user.id && message.author.id == "726392596183646260") return message.reply("B-Baka, it's not like I like it or anything >///<")
